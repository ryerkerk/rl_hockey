...
class Hockey2v2Roles(World):
    """
    World based on air hockey.

    This is a 2v2 version that uses self play. Separate models are used for each agent on a team. The left teams models
    are subject to training, while the right team uses previous snapshots of these models stored in a
    SelfPlayController object. Agent rewards are shaped such that one model should be pushed toward more defensive
    behavior, while the other model will tend toward more aggressive behavior.
    """
    def __init__(self):

        super(Hockey2v2Roles, self).__init__(world_size=[2000, 1000])
    ...
    def reset(self):
        ...
        # Player 2 (Left team, model-in-training separate from Player 1, expected to be more defensive)
        self.obj_list.append(ControlledCircle(CircleShape, x=[0.1 * w + gd, (1-p1y)*h], r=20, ang=0, mass=1,
                                              force_mag=1000, max_v=300, color=[80, 200, 255]))
        self.player_list.append(Player(obj=self.obj_list[-1], score=0, last_action=0,
                                control_func=self.cpu_controller[1].select_action, control_map=self.control_map[0]))

        # Player 3 (right agent, uses SelfPlayController containing snapshots of Player 1 model)
    ...
    def update_score(self):
        ...
        Player 1's score is shaped to encourage more aggressive behavior, while player 2's score is shaped for more
        defensive behavior. Scores for players 3 and 4 are not needed and not calculated.
        """
        puck_x = self.puck.x
        w, h = self.world_size

        p1_dist_x = np.minimum(w/2, w - puck_x[0])
        ...
        p2_dist_x = np.maximum(w / 2, w - puck_x[0])
    ...
    def get_state(self):
        ...
        p2_state[0, 8:10] = self.player_list[2].obj.x - ws / 2
        p2_state[0, 10:12] = self.player_list[2].obj.v
        p2_state[0, 12:14] = self.player_list[3].obj.x - ws / 2
        p2_state[0, 14:16] = self.player_list[3].obj.v
        ...
        p4_state[0, 8:10] = self.player_list[0].obj.x - ws / 2
        p4_state[0, 10:12] = self.player_list[0].obj.v
        p4_state[0, 12:14] = self.player_list[1].obj.x - ws / 2
        p4_state[0, 14:16] = self.player_list[1].obj.v
